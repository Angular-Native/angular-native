...
//!
//! The mirror of `an-ios`'s module of the same name, and it is a mirror rather
//! than shared code for the reason the whole crate is: `NSView` is not
//! `UIView`, and the one line where they differ is the type this returns.
//!
...
/// What the shell installs: a name in, a retained `NSView` out, or null for a
...
///
/// The symbol is the phone's, deliberately: the two never end up in the same
/// binary, and a Mac plugin's Swift is then the same line as an iPhone
/// plugin's.
///
/// # Safety
...
pub fn make(name: &str) -> Option<Retained<NSView>> {
