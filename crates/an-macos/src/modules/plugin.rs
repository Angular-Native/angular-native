...
//!
//! It is the same mechanism as `an-ios`'s, deliberately down to the symbol
//! names: the shell's registry, the dispatcher, the mailbox in `an-bridge` and
//! the four `an_plugin_*` entry points. A Mac plugin differs from a phone
//! plugin in what its Swift touches —`NSPasteboard` and not `UIPasteboard`—
//! and in nothing else, so a second design here would have been a second thing
...
/// The names registered so far. It is what decides whether the "no plugins
/// here" note is worth attaching to a rejection; see `modules::absent_note`.
...
pub fn pump() {
    ...
    let Some(dispatch) = *dispatch().lock().expect("poisoned dispatcher") else {
        ...
        for call in calls {
            let _ = bridge().reject(
                call.id,
                "the macOS shell did not install the plugin dispatcher",
            );
        }
        return;
    };
