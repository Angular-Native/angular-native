...
<SupportMatrix
  ...
  labels={{
    ...
    primitive: 'Primitive',
    full: 'Available',
    partial: 'Available, differently',
    none: 'Not available'
  }}
/>

`an-custom` is not in the table either, and for a different reason: it is not a
control this framework mounts but a hole a **plugin** mounts its own view
through, so what it becomes depends entirely on which plugin. It works
everywhere except watchOS, where there is no view hierarchy to put one in. See
